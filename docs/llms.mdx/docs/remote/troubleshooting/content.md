...
**Symptom**: You open Gent on both devices, but they do not appear in each other's sidebar.
...
2. **Multicast blocking**: Gent uses multicast UDP for discovery. Some enterprise networks, guest networks, and mesh routers block multicast traffic. Try a different network or check your router's settings.

3. **macOS local network permission**: On macOS, go to **System Settings > Privacy & Security > Local Network** and make sure Gent is allowed.

4. **Firewall**: Check that no firewall is blocking:
   * TCP port **53317** (or your configured port)
   * Multicast UDP traffic

5. **Restart both apps**: Close and reopen Gent on both devices. This triggers a fresh multicast scan.
...
2. **Sleep/standby**: When a device sleeps, the network connection drops. Gent reconnects automatically when the device wakes, but there may be a brief delay.
...
<Callout type="info">
  Gent uses exponential backoff for reconnection attempts. If the connection drops, it will try to reconnect automatically — starting at 10 seconds and backing off to 80 seconds between attempts.
</Callout>
...
**Symptom**: Gent fails to start or pair, with errors about port 53317.

**Fix**: Another application is using port 53317. You can change Gent's port in **Settings**. Make sure both devices use the same port.
...
2. **Localhost binding**: Some dev servers bind to `127.0.0.1` only. Gent's reverse proxy handles this, but check that the server is running
...
* Verify that both devices are running the same version of Gent
